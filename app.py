...
def get_minmax(nbds):
    df_state = df[df["nbhid"].isin(nbds)]  # pick one neighborhood
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            ...
            ), colorbar, nbd_colorbar, info]),
            html.Div([dd_state, dd_csc],
                     style={"position": "relative", "bottom": "80px", "right": "10px", "z-index": "1000", "width": "200px"})
...
def update_trends_graph(years_range, months_range, nbds):
...
@app.callback([Output("geojson", "hideout"), Output("geojson", "data"), Output("colorbar", "colorscale"),
               ...
               Output("outlines", "hideout"), Output("outlines", "data"), ],
              #    Output("outline_colorbar", "categories")],
              [Input('year_slider', 'value'), Input("dd_csc", "value"), Input("dd_state", "value")])
def update_map(year_slider, csc, nbds):
    nbds = list(map(int, nbds))
    csc, data, mm = json.loads(csc), get_data(
    ...
    return hideout, data, csc, mm["min"], mm["max"], outline_hideout, outline_data
...
def func(n_clicks, years_range, nbds):
    if n_clicks:
        df_nbh = df[df["nbhid"].isin(nbds)]  # filter chosen states
        nbhname = df_nbh['nbh_name'].loc[0]
        df_nbh = df_nbh[(df_nbh['CREATION YEAR'] <= years_range[1]) & (
            df_nbh['CREATION YEAR'] >= years_range[0])]
        return send_data_frame(df_nbh.to_csv, "".join(["kc311", str(nbds), '_', nbhname,
